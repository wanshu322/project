# File name: A1002BattleforCrymland_v1.0.py
# Author: Wanshu Wang
# Student ID# 1818536
# Date: 03.16.2021
...
def main():
    ...
    while True:
        ...
        if exit not in ['Y','N','y','n']:   #check if the input is valid
              ...
              #initial dataframe for weekly stats
              col_nm_list = ['Bigg_Jailed','BiggWealth','JailedNum','BribeAmount','ActorsNum']
              d = pd.DataFrame(data = None,index = None,columns = col_nm_list)
              ...
              while stats_weekly.Bigg_Capture == False and stats_weekly.Bigg_week.week_number < weeks:
                  ...
                  week_num = stats_weekly.Bigg_week.week_number
                  #col : ['Week#','Bigg_Jailed','BiggWealth','JailedNum','BribeAmount','ActorsNum']
                  d.loc[week_num] = [stats_weekly.Bigg_Capture,stats_weekly.BiggWealth,stats_weekly.tot_jailed,stats_weekly.amount_bribed,stats_weekly.n_actors] # don't add  stats_weekly.Bigg_week.week_number, 

                  outfile.write(stats_weekly.generateStats())

              #create a dataframe and write it into a csv file
              #d.append(P_data)
              print(d)
              d.to_csv('BC_WeeklyData'+str(test_num)+'.csv')
              outfile.close()
             
              
              #plot
              #d = pd.read_csv('BC_WeeklyData.csv')  
              fig, ax = plt.subplots()
              ax3 = ax.twinx()
